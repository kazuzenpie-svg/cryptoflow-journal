-- CryptoFlow Journal Database Setup
-- Run this in your Supabase SQL Editor to ensure all tables and functions are set up correctly
...
DROP POLICY IF EXISTS "Investors can read trader profiles for binding" ON public.users;

DROP POLICY IF EXISTS "Traders can manage their own trades" ON public.trades;
DROP POLICY IF EXISTS "Investors can view bound trader's trades" ON public.trades;
DROP POLICY IF EXISTS "Investors can view approved trader trades" ON public.trades;
...
DROP POLICY IF EXISTS "Investors can create binding requests" ON public.bindings;
DROP POLICY IF EXISTS "Traders can update binding status" ON public.bindings;
DROP POLICY IF EXISTS "Investors can view their own bindings" ON public.bindings;
DROP POLICY IF EXISTS "Traders can view their investor bindings" ON public.bindings;
...
-- Allow investors to read trader profiles for binding
CREATE POLICY "Investors can read trader profiles for binding" ON public.users
FOR SELECT USING (role = 'trader' OR auth.uid() = id);
...
-- Fixed investor access policy for trades
...
-- Function to handle binding approval
...
DROP TRIGGER IF EXISTS update_bindings_updated_at ON public.bindings;
DROP TRIGGER IF EXISTS audit_trades_trigger ON public.trades;
DROP TRIGGER IF EXISTS check_binding_limit_trigger ON public.bindings;
DROP TRIGGER IF EXISTS validate_investment_pnl_trigger ON public.trades;
...
-- Success message
SELECT 'Database setup completed successfully! All tables, functions, triggers, and RLS policies are now in place.' as message;